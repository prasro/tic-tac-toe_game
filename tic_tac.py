...
# o decide the turn of which player
turn_of_player = 0
    ...
    for let in ["O", "X"]:
        for i in possiblemove:
            ...
            if winner(boardcopy, let):
                return i
    if corner := [i for i in possiblemove if i in [[0, 0], [0, 2], [2, 0], [2, 2]]]:
...
def get_text_pc(i, j, gb, l1, l2):
    ...
    x = True
    if winner(board, "X"):
        x = destroy_game_and_show_meaage(gb, "Winner", "Player won the match")
    elif winner(board, "O"):
        x = destroy_game_and_show_meaage(gb, "Winner", "Computer won the match")
    elif isfull():
        x = destroy_game_and_show_meaage(gb, "Tie Game", "Tie Game")
    if x and turn_of_player % 2 != 0:
        move = pc()
        button[move[0]][move[1]].config(state=DISABLED)
        get_text_pc(move[0], move[1], gb, l1, l2)


def destroy_game_and_show_meaage(gb, arg1, arg2):
    gb.destroy()
